...
const Reveal: React.FC<RevealProps> = ({ web3 }) => {
  ...
  useEffect(() => {
    const fetchRevealDomains = async () => {
  ...
  const revealHandler = async () => {
    try {
      ...
      contract.on("Revealed", (from, domain) => {
        console.log(`Revealed event triggered!`);
        console.log(`From: ${from}`);
        console.log(`Domain: ${domain}`);
        window.location.reload();
      });
      const tx = await contract.reveal(domain, amountBytes, secretBytes);
      await tx.wait();
    } catch (e) {
      toast({
        variant: "destructive",
    ...
    <div className='absolute flex h-full w-full flex-col bg-white'>
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="flex flex-col gap-y-4 w-2/5">
          ...
          <Button type="submit" onClick={() => revealHandler()}>Commit</Button>
        </div>
      </div>
    </div>
  )
